...
use Fiserv\Payments\Model\Adapter\PayPal\PayPalTokenizationRequest;
...
use Fiserv\Payments\Gateway\Response\PayPal\PayPalVaultDetailsHandler;
...
/**
 * Class TokenizePaypalSession
 */
class TokenizePaypalSession extends Action implements CsrfAwareActionInterface, HttpPostActionInterface
{
	const SESSION_ID_KEY = "session_id";
	const WEBSITE_ID_KEY = "website_id";
	const CUSTOMER_ID_KEY = "customer_id";
	const CARD_ALREADY_ADDED_IN_VAULT = 'PayPal payment token already exists in vault';
	...
	/**
	 * @var PayPalTokenizationRequest
	 */
	private $payPalAdapter;

	/**
	 * @var PayPalVaultDetailsHandler
	 */
	...
	* @param PayPalTokenizationRequest $payPalAdapter
	*/
       public function __construct(
	       Context $context,
	PayPalTokenizationRequest $payPalAdapter,
		   PayPalVaultDetailsHandler $vaultHandler,
	...
	try {
		...
		if ($sessionId == null || $customerId == null) {
			return $this->processBadRequest($response); 
			   ...
			   $tokenResponse = $this->payPalAdapter->tokenizeSession($sessionId);
		...
		if(isset($tokenResponse["cardDetails"]) && isset($tokenResponse["cardDetails"][0]) && self::PIN_ONLY === strtolower($tokenResponse["cardDetails"][0]["detailedCardProduct"]) ) {
	       ...
	       {
		       $this->logger->logInfo(1, "PayPal token not tokenized. Already stored");
