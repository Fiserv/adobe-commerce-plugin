...
class NetworkDetails implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
    ...
    {
        if (!is_null($debit_network_id) && (mb_strlen($debit_network_id) > 64)) {
    ...
    {
        if (!is_null($transaction_sequence) && (mb_strlen($transaction_sequence) > 64)) {
    ...
    {
        if (!is_null($system_trace) && (mb_strlen($system_trace) > 64)) {
    ...
    {
        if (!is_null($network_response_code) && (mb_strlen($network_response_code) > 16)) {
    ...
    {
        if (!is_null($network_avs_code) && (mb_strlen($network_avs_code) > 1)) {
    ...
    {
        if (!is_null($pan_mapping_account_number) && (mb_strlen($pan_mapping_account_number) > 19)) {
     ...
     *
     * @param string|null $cardholder_response Non-normalized response from for verifying cardholder data where each position represents the postal/zip code, address, name, telephone, and email respectively.  Valid Values: * Y = Yes data matches * N = No data does not match * U - Data unchecked * R - Retry * S - Service not allowed * Blank - Data not sent
     *
     * @return self
     */
    public function setCardholderResponse($cardholder_response)
    {
        if (!is_null($cardholder_response) && (mb_strlen($cardholder_response) > 5)) {
     ...
     * Sets debit_routing
     *
     * @param string|null $debit_routing Provides the Debit network routing Type for PINless and Signature Debit transactions.  Valid Values: * CREDIT * DEBIT * DUAL
     *
     * @return self
     */
    public function setDebitRouting($debit_routing)
    {
        if (!is_null($debit_routing) && (mb_strlen($debit_routing) > 256)) {
