...
class AuthorizeResponseValidator extends TransactionResponseValidator
{
    ...
    public function validate(array $validationSubject): ResultInterface
    {
        ...
        if (!$this->isStatusSuccessful($statusCode)) {
            array_push($errorMessages, "PayPal authorization failed: Invalid HTTP status code " . $statusCode);
        ...
        if (!$this->isStateSuccessful($transactionState)) {
            array_push($errorMessages, "PayPal authorization failed: Transaction state is " . ($transactionState ?? "unknown"));
