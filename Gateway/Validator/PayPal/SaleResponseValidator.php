...
class SaleResponseValidator extends TransactionResponseValidator
{
    ...
    public function validate(array $validationSubject): ResultInterface
    {
        ...
        if (!$this->isStatusSuccessful($statusCode)) {
            array_push($errorMessages, "PayPal sale failed: Invalid HTTP status code " . $statusCode);
        ...
        if (!$this->isStateSuccessful($transactionState)) {
            array_push($errorMessages, "PayPal sale failed: Transaction state is " . ($transactionState ?? "unknown"));
