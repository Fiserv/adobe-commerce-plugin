    ...
    return Component.extend({
        ...
        selectPazePaymentMethod: async function () {
            ...
            try {
                ...
                console.log('[Paze] Submitting Paze payment method...');
                const submitResult = await this.pazeComponent.submit(params);
                console.log('[Paze] Payment method submitted successfully:', submitResult);
        ...
        onPazeSuccess: function (selectResult, submitResult) {
            ...
            // Automatically place the order since Paze handles the payment confirmation
        ...
        placeOrderCallback: function () {
            require(['Magento_Checkout/js/action/place-order'], (placeOrderAction) => {
                placeOrderAction(this.getData()).done(() => {
                    ...
                    console.error('[Paze] Order placement failed:', error);
                    globalMessageList.addErrorMessage({ 
                        message: $t('Failed to place order. Please try again.') 
                    });
                });
            });
        },
